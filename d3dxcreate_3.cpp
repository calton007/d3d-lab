...
bool Display(float timeDelta)
{
	if (Device)
	{
		...
		else if (::GetAsyncKeyState(VK_F2) & 0x8000f){
			flag = false;
			flag1 = true;
		}
		else if (::GetAsyncKeyState(VK_F3) & 0x8000f){
			flag = false;
			flag1 = false;
		}
		if (flag){
			Device->SetLight(0, &dirLight[0]);
		}
		else if (flag1){
			Device->SetLight(0, &dirLight[1]);
		}
		else{
			Device->SetLight(0, &dirLight[2]);
		}
		//Device->SetLight(0, &dirLight[index%3]);
		//Device->SetLight(0, &dirLight);
		// Animate the camera:
